...
// - Adds subscriber to Beehiiv (v2 API) with state + anonymous behavioral tags
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const customFields = [
      ...
      ...(usage?.grade ? [{ name: 'grade', value: String(usage.grade) }] : []),
      ...(usage?.period ? [{ name: 'period', value: usage.period }] : []),
      ...(usage?.verdict ? [{ name: 'verdict', value: usage.verdict }] : []),
      ...(usage?.gap_from_p50 != null ? [{ name: 'gap_from_p50', value: String(usage.gap_from_p50) }] : []),
      ...(usage?.scores_entered != null ? [{ name: 'scores_entered', value: String(usage.scores_entered) }] : []),
    ];
